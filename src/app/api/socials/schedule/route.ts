...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Starting scheduled socials detection run...')

    // Run socials detection for all active profiles
    const result = await socialsDetectionService.runDetection({
      runType: 'scheduled'
    })

    console.log(`Scheduled run completed: ${result.summary.changesFound} changes found across ${result.summary.profilesScraped} profiles`)
    ...
    console.error('Scheduled socials detection failed:', error)
    return NextResponse.json(
      {
        success: false,
